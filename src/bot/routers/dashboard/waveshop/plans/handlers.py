...
from src.bot.states import WaveshopPlans
...
async def on_plan_select(
    ...
    if not plan:
        raise ValueError(
            f"Attempted to select non-existent plan '{sub_manager.item_id}'"
        )
    ...
    await sub_manager.switch_to(state=WaveshopPlans.CONFIGURATOR)
...
async def on_plan_remove(
    ...
    if is_double_click(
        sub_manager.manager, key=f"delete_confirm_{plan_id}", cooldown=10
    ):
    ...
    logger.debug(
        f"{log(user)} Clicked delete for plan ID '{plan_id}' (awaiting confirmation)"
    )
...
async def on_name_input(
    ...
    if await plan_service.get_by_name(plan_name=message.text):
        logger.warning(
            f"{log(user)} Tried to set duplicate plan name '{message.text}'"
        )
        await notification_service.notify_user(
            user=user,
            payload=MessagePayload(i18n_key="ntf-plan-invalid-name"),
    ...
    await dialog_manager.switch_to(state=WaveshopPlans.CONFIGURATOR)
...
async def on_availability_select(
    ...
    logger.info(
        f"{log(user)} Successfully updated plan availability to '{plan.availability}'"
    )
    await dialog_manager.switch_to(state=WaveshopPlans.CONFIGURATOR)
...
async def on_active_toggle(
    ...
    adapter.save(plan)
    logger.info(
        f"{log(user)} Successfully toggled plan active status to '{plan.is_active}'"
    )
...
async def on_traffic_input(
    ...
    logger.info(
        f"{log(user)} Successfully set plan traffic limit to '{plan.traffic_limit}'"
    )
    await dialog_manager.switch_to(state=WaveshopPlans.CONFIGURATOR)
...
async def on_devices_input(
    ...
    logger.info(
        f"{log(user)} Successfully set plan device limit to '{plan.device_limit}'"
    )
    await dialog_manager.switch_to(state=WaveshopPlans.CONFIGURATOR)
...
async def on_duration_select(
    ...
    await sub_manager.switch_to(state=WaveshopPlans.PRICES)
...
async def on_duration_remove(
    ...
    adapter.save(plan)
    logger.info(
        f"{log(user)} Successfully removed duration '{duration_to_remove}' days from plan"
    )
...
async def on_duration_input(
    ...
    if message.text is None or not (
        ...
        logger.warning(
            f"{log(user)} Provided invalid duration input: '{message.text}'"
        )
        await notification_service.notify_user(
            user=user,
            payload=MessagePayload(i18n_key="ntf-plan-invalid-number"),
    ...
    await dialog_manager.switch_to(state=WaveshopPlans.DURATIONS)
...
async def on_currency_select(
    ...
    await dialog_manager.switch_to(state=WaveshopPlans.PRICE)
...
async def on_price_input(
    ...
    adapter.save(plan)
    await dialog_manager.switch_to(state=WaveshopPlans.PRICES)
...
async def on_allowed_user_input(
    ...
    if allowed_user.telegram_id in plan.allowed_user_ids:
        logger.warning(
            f"{log(user)} User '{allowed_user.telegram_id}' is already allowed for plan"
        )
        await notification_service.notify_user(
            user=user,
...
async def on_squads(
    ...
    await dialog_manager.switch_to(state=WaveshopPlans.SQUADS)
...
async def on_confirm_plan(  # noqa: C901
    ...
    if plan_dto.id:
        ...
        existing_plan: Optional[PlanDto] = await plan_service.get_by_name(
            plan_name=plan_dto.name
        )
        if existing_plan:
            logger.warning(
                f"{log(user)} Plan with name '{plan_dto.name}' already exists. Aborting"
            )
            await notification_service.notify_user(
                user=user,
    ...
    await dialog_manager.start(state=WaveshopPlans.MAIN)
