...
from src.bot.states import WaveshopGateways
...
async def on_gateway_select(
    ...
    await sub_manager.switch_to(state=WaveshopGateways.SETTINGS)
...
async def on_gateway_test(
    ...
    try:
        payment = await payment_gateway_service.create_test_payment(
            user, gateway.type
        )
...
async def on_field_select(
    ...
    await dialog_manager.switch_to(state=WaveshopGateways.FIELD)
...
async def on_field_input(
    ...
    if not gateway or not gateway.settings:
        await dialog_manager.switch_to(state=WaveshopGateways.MAIN)
    ...
    await payment_gateway_service.update(gateway)
    await dialog_manager.switch_to(state=WaveshopGateways.SETTINGS)
