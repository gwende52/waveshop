...
def get_banner(
    ...
    for current_locale in [locale, default_locale]:
        ...
        for format in BannerFormat:
            ...
            logger.debug(
                f"Found banner '{name}' in locale '{current_locale}': '{path}'"
            )
            return path, content_type

    logger.warning(
        f"Banner '{name}' not found in locales '{locale}' or '{default_locale}'"
    )
...
class Banner(StaticMedia):
    def __init__(self, name: BannerName) -> None:
        ...
        super().__init__(
            path="path", url=None, type=ContentType.UNKNOWN, when=_is_use_banners
        )
