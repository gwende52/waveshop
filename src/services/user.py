...
class UserService(BaseService):
    ...
    async def create(self, aiogram_user: AiogramUser) -> UserDto:
        user = UserDto(
            ...
            role=(
                UserRole.DEV
                if self.config.bot.dev_id == aiogram_user.id
                else UserRole.USER
            ),
            language=(
    ...
    async def compare_and_update(
        ...
        if user.name != new_name:
            logger.debug(
                f"User '{user.telegram_id}' name changed ({user.name} -> {new_name})"
            )
            user.name = new_name
    ...
    async def search_users(self, message: Message) -> list[UserDto]:
        ...
        if message.forward_from and not message.forward_from.is_bot:
            ...
            if single_user:
                ...
                logger.info(
                    f"Search by forwarded message, found user '{target_telegram_id}'"
                )
            else:
                logger.warning(
            ...
            if search_query.isdigit():
                ...
                if single_user:
                    found_users.append(single_user)
                    logger.info(
                        f"Searched by Telegram ID '{target_telegram_id}', user found"
                    )
                else:
                    logger.warning(
                ...
                try:
                    ...
                    if single_user:
                        found_users.append(single_user)
                        logger.info(
                            f"Searched by Waveshop ID '{target_id}', user found"
                        )
                    else:
                        logger.warning(
                            f"Searched by Waveshop ID '{target_id}', user not found"
                        )
                except (IndexError, ValueError):
                    logger.warning(
                        f"Failed to parse Waveshop ID from query '{search_query}'"
                    )
    ...
    async def set_current_subscription(
        self, telegram_id: int, subscription_id: int
    ) -> None:
        await self.uow.repository.users.update(
        ...
        logger.info(
            f"Set current_subscription='{subscription_id}' for user '{telegram_id}'"
        )
