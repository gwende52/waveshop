...
from src.bot.keyboards import get_waveshop_keyboard
...
class NotificationService(BaseService):
    ...
    async def notify_user(
        ...
        if ntf_type and not await self.settings_service.is_notification_enabled(
            ntf_type
        ):
            logger.debug(
    ...
    async def system_notify(
        ...
        if not await self.settings_service.is_notification_enabled(ntf_type):
            logger.debug(
                "Skipping system notification: notification type is disabled in settings"
            )
            return []
    ...
    async def waveshop_notify(self) -> bool:
        dev = (
            await self.user_service.get(self.config.bot.dev_id)
            or self._get_temp_dev()
        )
        payload = MessagePayload(
            i18n_key="ntf-waveshop-info",
            i18n_kwargs={"version": __version__},
            reply_markup=get_waveshop_keyboard(),
    ...
    async def _send_message(
        self, user: UserDto, payload: MessagePayload
    ) -> Optional[Message]:
        try:
    ...
    def _get_close_notification_button(self, locale: Locale) -> InlineKeyboardButton:
        ...
        return InlineKeyboardButton(
            text=button_text, callback_data=Notification.CLOSE.state
        )
    ...
    def _translate_keyboard_texts(
        self, keyboard: AnyKeyboard, locale: Locale
    ) -> AnyKeyboard:  # noqa: C901
        if isinstance(keyboard, InlineKeyboardMarkup):
            ...
            for row in keyboard.keyboard:
                ...
                for button in row:
                    if button.text:
                        try:
                            button.text = self._get_translated_text(
                                locale, button.text
                            )
                        except Exception:
