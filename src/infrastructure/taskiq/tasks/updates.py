...
from src.bot.keyboards import get_waveshop_update_keyboard
...
REMOTE_VERSION_URL: Final[str] = (
    "https://raw.githubusercontent.com/snoups/waveshop/main/src/__version__.py"
)
...
async def check_bot_update() -> None:
    async with httpx.AsyncClient() as client:
        ...
        remote_version = (
            resp.text.strip().split("=")[-1].strip().strip('"').strip("'")
        )
    ...
    try:
        ...
        if rv > lv:
            logger.info(
                f"New version available: {remote_version} (local: {local_version})"
            )
            await send_system_notification_task.kiq(
                ...
                payload=MessagePayload.not_deleted(
                    ...
                    reply_markup=get_waveshop_update_keyboard(),
                ),
            )
        elif rv == lv:
            ...
            logger.debug(
                f"Local version is ahead of remote ({local_version} > {remote_version})"
            )
